  ...
  module Hashable
    ...
    def from_hash(original_hash)
      ...
      pruned_hash&.each_key do |key|
        ...
        klass = version_class::PRIMITIVES.key?(meta['type']) ? nil : version_class.const_get(meta['type'])
        if !klass.nil? && !value.nil?
          ...
          instance_variable_set("@#{local_name}", value)
        elsif !version_class::PRIMITIVES.include?(meta['type']) && meta['type'] != 'xhtml'
    ...
    def make_child(child, klass)
      ...
      if child['resourceType'] && !klass::METADATA['resourceType']
        klass = begin
          version_class.const_get(child['resourceType'])
    ...
    def convert_primitive(value, meta)
      ...
      elsif version_class::PRIMITIVES.include?(meta['type'])
